...
(def actual-input ["bvhfawknyoqsudzrpgslecmtkj"
...
(deftest checksum
  ...
  (testing "checksum of case one"
    (is (= 6696 (get-checksum actual-input)))))
...
(defn check-difference-pair [a b]
  (let [difference (diff (seq (char-array a)) (seq (char-array b)))
    ...
    (if (and (= 1 (count (first non-nil-diff))) (= 1 (count (second non-nil-diff))))
      (string/join (last non-nil-diff)))))
...
(deftest find-difference
  ...
  (testing "find the common between two strings"
    (is (= "fgij" (check-difference-pair "fghij" "fguij")))))

(defn find-common [data]
  (if (empty? data)
    nil
    (let [first (first data)]
      (if-let [match (some #(and (check-difference-pair first %) %) (rest data))]
        (check-difference-pair first match)
        (recur (rest data))))))

(deftest find-common-between-correct-ids
  (testing "find common using sample data"
    (let [sample-data ["abcde" "fghij" "klmno" "pqrst" "fguij" "axcye" "wvxyz"]]
      (is (= "fgij" (find-common sample-data)))
      (is (= "bvnfawcnyoeyudzrpgslimtkj" (find-common actual-input))))))